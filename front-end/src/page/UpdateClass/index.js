  ...
  function UpdateClass() {
    ...
    async function getDetailClass(){
        try{
            ...
            const response = await api.get(`api/v1/classes/update/${idClass}`);
            console.log(response);
            setFormCreateClass({
                ...response.data,
                commissionFeeFormatted: formatCurrency(response.data.commissionFee),
                salaryFormatted: formatCurrency(response.data.salary),
                classDepositFormatted: formatCurrency(response.data.classDeposit),
            });
            Array.from(response.data.dateAndTimeDtoList).forEach((_) => setRows(rows + 1));
        }catch(e){
    ...
    async function createClass() {
      try {
        dispatch(openBackDrop());
        await api.put(`api/v1/classes`, formCreateClass);
        showSnackbar("Cập nhật thành công");
        navigate("/class/" + idClass);
      } catch (e) {
                        ...
                        {item.title1 !== "Trạng thái" ? (
                          item.title1 === "Môn học" ? (
                            ...
                            <input
                              ...
                              defaultValue={formCreateClass.studentPhoneNumber}
                              onChange={handChangeForm}
                            />
                          )
                        ) : (
                      ...
                      >
                        {item.title2 === "Kiểu dạy" ? (
                          ...
                          <input
                            type="date"
                            name="dateStart"
                            value={formatDay(formCreateClass.dateStart)}
                            onChange={handChangeForm}
                          />
                          ...
                          <input
                            ...
                            defaultValue={formCreateClass.tutorPhoneNumber}
                            onChange={handChangeForm}
                          />
                        )}
                      </Grid>
                      ...
                      >
                        {item.title === "Địa chỉ" ? (
                          <Select
                            ...
                            displayEmpty
                          >
                            {console.log(formCreateClass.addrId)}
                            <MenuItem
                              value={null}
                              sx={{ background: "#E0BBE4 !important" }}
                            >
                            ...
                            {addresses.map((it) => (
                              <MenuItem
                                ...
                                sx={{ background: "#E0BBE4 !important" }}
                              >
                                {formCreateClass.addrId}
                                {it.houseNumber + ", " + it.streetName}
                              </MenuItem>
                            ))}
                          </Select>
                        ) : (
                          <input name="requirements" defaultValue={formCreateClass.requirements} onChange={handChangeForm} />
                        )}
                      </Grid>
                    </Grid>
                  </>
